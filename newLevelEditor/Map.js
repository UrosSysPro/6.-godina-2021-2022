class Map{
    ...
    setUpInputs(){
        ...
        for(let i=0;i<2;i++){
            ...
            inputs[2+i].addEventListener("input",function(event){
                if(map.currentSelectedIndex==-1||map.currentSelectedIndex==-2)return;
                let y=parseInt(event.target.value);
                map.items[map.currentSelectedIndex].y=y;
                map.items[map.currentSelectedIndex].update();
                map.setInputsForCurrentIndex();
            });
            inputs[4+i].addEventListener("input",function(event){
                if(map.currentSelectedIndex==-1||map.currentSelectedIndex==-2)return;
                let w=parseInt(event.target.value);
                map.items[map.currentSelectedIndex].w=w;
                map.items[map.currentSelectedIndex].update();
                map.setInputsForCurrentIndex();
            });
            inputs[6+i].addEventListener("input",function(event){
                if(map.currentSelectedIndex==-1||map.currentSelectedIndex==-2)return;
                let h=parseInt(event.target.value);
                map.items[map.currentSelectedIndex].h=h;
                map.items[map.currentSelectedIndex].update();
                map.setInputsForCurrentIndex();
            });
        }
    ...
    toString(){
        ...
        for(let i=0;i<this.items.length;i++){
            toReturn.walls.push({
                x:this.items[i].x+this.items[i].w/2,
                y:this.items[i].y+this.items[i].h/2,
                w:this.items[i].w/2,
                h:this.items[i].h/2,
            });
        }
